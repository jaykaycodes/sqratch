...
pub trait DbApi {
    ...
    async fn get_all_entities<R: Runtime>(window: Window<R>) -> Result<Vec<Entity>, AppError>;

    // EVENTS

    #[taurpc(event)]
    async fn subscribe_connection_status<R: Runtime>(
        window: Window<R>,
...
impl DbApi for DbApiImpl {
    ...
    async fn get_all_entities<R: Runtime>(
        self,
        window: Window<R>,
