	...
	return (
		<div className="h-full overflow-x-clip overflow-y-auto">
			<Show else={<Empty text={emptyText} />} ifReady={items$}>
				<For each={items$} item={WorkbenchItemRow} itemProps={{ isPending: state === 'pending' }} />
			</Show>
		</div>
	)
}
...
function Empty({ text }: { text: string }) {
	return (
		<div className="flex p-2 items-center justify-center h-16 text-muted-foreground">{text}</div>
	)
}
